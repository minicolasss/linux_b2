...
# --- 1. SAUVEGARDE DATABASES (MARIADB) ---
echo "Export de la base de données..." >> "$LOG_FILE"
# On exporte TOUTES les bases, on compresse en GZIP pour gagner de la place
if mysqldump --all-databases --single-transaction --quick --lock-tables=false | gzip > "$DB_DIR/alldatabases_$DATE.sql.gz"; then
    echo "Dump SQL réussi." >> "$LOG_FILE"
else
    echo "ERREUR lors du Dump SQL." >> "$LOG_FILE"
fi

# --- 2. CRÉATION DE L'ARCHIVE SYSTÈME ---
# On inclut maintenant le dossier /backup/database dans l'archive globale !
echo "Compression des fichiers..." >> "$LOG_FILE"
...
# --- 3. NETTOYAGE DES VIEUX SQL ---
# On supprime aussi les vieux fichiers .sql.gz pour ne pas saturer le disque
find "$DB_DIR" -name "alldatabases_*.sql.gz" -mtime +$RETENTION_DAYS -exec rm {} \; >> "$LOG_FILE" 2>&1

# --- VÉRIFICATION ET MAIL ---
if [ $TAR_STATUS -eq 0 ] || [ $TAR_STATUS -eq 1 ]; then
    echo "Sauvegarde terminée avec succès : $FILENAME" >> "$LOG_FILE"
    
    # Nettoyage des vieilles archives TAR
    find "$BACKUP_DIR" -name "backup-srv-web-*.tar.gz" -mtime +$RETENTION_DAYS -exec rm {} \; >> "$LOG_FILE" 2>&1
    
    # Envoi du Mail
    SIZE=$(du -h "$BACKUP_DIR/$FILENAME" | cut -f1)
    echo "Sauvegarde Complète (Système + SQL) OK. Fichier: $FILENAME. Taille: $SIZE." | mail -s "[OK] Full Backup Web Server" "$EMAIL"
else
